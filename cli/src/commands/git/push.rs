...
///
/// By default, pushes tracking bookmarks and tags defined by
/// `revsets.git-push` revset. Use `--bookmark`/`--tag` to push
...
async fn find_default_target_revisions(
    ...
    if commit_ids.as_mut().peek().await.is_none() {
        writeln!(
            ui.warning_default(),
            "No bookmarks/tags found in the default push revset: {push_revset} for {remote}",
            remote = remote.as_symbol()
        )?;
    }
